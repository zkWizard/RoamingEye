 ...
 *
 * These are sobering by design, and layer-dependent. Where a legend gradient
 * is anchored on the hues GIBS actually renders, inversion is tight: aerosol
 * (RMSE 0.13) and 2 m air temperature (1.03 K) recover every colormap colour.
 * Where it is a coarse freehand approximation it is not: precipitation and
 * soil recover loosely, and LST's gradient misses GIBS's cold-end hues
 * entirely (all-null). On the loose layers the probe is reliable for
 * *relative* analysis (trends, anomalies, seasonality — scale-monotone-robust),
 * not absolute values. Re-anchoring the remaining gradients on the real GIBS
...
> = {
  ...
  // Re-measured 2026-08-11 after re-anchoring the gradient on GIBS's ramp
  // (was rmse 18.95, nulls 44 — the old cold stop sat on the under-range cap).
  airtemp: { rmse: 1.03, nulls: 0, total: 90 },
