...
import {
  SOIL_MOISTURE_RECORD_LIMITATIONS,
  SOIL_MOISTURE_RECORD_METRIC,
...
const priorYears = (
  ...
  values.map((value, index) =>
    soilMonth(2015 + index, month, value, validFraction)
  );
...
describe("soil moisture same-month record margin", () => {
  it("flags a new wettest-in-record month with its exceedance margin and holder", () => {
    const result = describeSoilMoistureRecordMargin(
      soilMonth(2025, 7, 160),
      JULY_RECORD,
      AVAILABLE_THROUGH
    );
    ...
    expect(result).toMatchObject({
      ...
      sampleCount: 10,
      targetValue: 160,
      priorWettestValue: 150,
      priorWettestMonth: { year: 2024, month: 7 },
      priorDriestValue: 100,
      priorDriestMonth: { year: 2015, month: 7 },
      standing: "wettest-in-record",
      recordExceedanceMargin: 10,
      reason: null,
    });
    // Below the (now-broken) wet record is negative; above the dry record wide.
    expect(result.marginBelowWettest).toBe(-10);
    expect(result.marginAboveDriest).toBe(60);
    expect(result.metric).toBe(CLIMATE_METRICS["soil-moisture"]);
    expect(result.metric).toBe(SOIL_MOISTURE_RECORD_METRIC);
    expect(result.limitations).toBe(SOIL_MOISTURE_RECORD_LIMITATIONS);
  });
  ...
  it("flags a new driest-in-record month with its exceedance margin", () => {
    ...
    expect(result.marginAboveDriest).toBe(-10);
    expect(result.marginBelowWettest).toBe(60);
  });

  it("reports both margins for a value within the observed same-month range", () => {
    const result = describeSoilMoistureRecordMargin(
      soilMonth(2025, 7, 130),
      JULY_RECORD,
      AVAILABLE_THROUGH
    );
    ...
    expect(result.marginBelowWettest).toBe(20);
    expect(result.marginAboveDriest).toBe(30);
  });

  it("ties the wet record without breaching it (record has spread)", () => {
    const result = describeSoilMoistureRecordMargin(
      soilMonth(2025, 7, 150),
      JULY_RECORD,
      AVAILABLE_THROUGH
    );
  ...
  it("ties the dry record without breaching it (record has spread)", () => {
    ...
    expect(result.marginBelowWettest).toBe(50);
  });
  ...
  it("ties a flat record (no spread) at both extremes at once", () => {
    ...
    expect(result).toMatchObject({
      ...
      priorWettestValue: 120,
      priorDriestValue: 120,
      recordExceedanceMargin: null,
    });
    expect(result.marginBelowWettest).toBe(0);
    expect(result.marginAboveDriest).toBe(0);
  ...
  it("resolves tied extremes to the earliest holder month", () => {
    ...
    const result = describeSoilMoistureRecordMargin(
      soilMonth(2025, 9, 160),
      record,
      AVAILABLE_THROUGH
    );

    expect(result.priorWettestValue).toBe(150);
    expect(result.priorWettestMonth).toEqual({ year: 2016, month: 9 });
    expect(result.priorDriestValue).toBe(100);
    expect(result.priorDriestMonth).toEqual({ year: 2015, month: 9 });
  });

  it("excludes the target year from its own record", () => {
    // A 2020 duplicate of the target year must not seed the baseline.
    const withTargetYear = [...JULY_RECORD, soilMonth(2025, 7, 500)];
    const result = describeSoilMoistureRecordMargin(
      soilMonth(2025, 7, 160),
      withTargetYear,
      AVAILABLE_THROUGH
