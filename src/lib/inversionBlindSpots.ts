 ...
 * alongside it. Together they say what neither says alone: aerosol's RMSE of
 * 0.13 is a whole-ramp figure, whereas soil's 8.23 kg/m², precipitation's
 * 20.36 mm/day and air temperature's 18.95 K are *survivor-only* numbers,
 * measured on palettes where our gradient cannot read half the range at all.
 *
 * The findings are unflattering by design, and each names a concrete downstream
 * hazard:
 *  - `airtemp` is blind across 238.5–281.5 K (−34.6 to +8.4 °C) — a 48% hole
 *    covering most inhabited cool-season temperatures, so a mid-latitude
 *    winter series keeps only the months warm enough to invert.
 *  - `precip` is blind from the ramp's floor up to 19.4 mm/day, so ordinary
 *    rainfall drops out and only the heaviest months survive.
 *  - `soil` loses both the dry end (0.5–11.5 kg/m²) and a 30% mid-range band.
 *  - `sst` is censored at both ends and across 17.9–24.4 °C.
...
> = {
  ...
  airtemp: {
    ...
    recovered: 46,
    total: 90,
    widest: { lo: 238.5, hi: 281.5, unit: "K" },
  },
  sst: {
    shape: "end-truncated",
    recovered: 128,
    total: 213,
    widest: { lo: 17.93, hi: 24.38, unit: "°C" },
  },
  precip: {
    shape: "end-truncated",
    recovered: 27,
    total: 50,
    widest: { lo: 0.43, hi: 19.44, unit: "mm/day" },
  },
  soil: {
    shape: "end-truncated",
    recovered: 21,
