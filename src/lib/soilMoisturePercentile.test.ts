...
import {
  SOIL_MOISTURE_PERCENTILE_LIMITATIONS,
  SOIL_MOISTURE_PERCENTILE_METRIC,
...
describe("soil moisture percentile of record", () => {
  it("ranks a target within its same-calendar-month record via the mid-rank convention", () => {
    ...
    const result = describeSoilMoisturePercentile(
    ...
    expect(result.metric).toBe(SOIL_MOISTURE_PERCENTILE_METRIC);
    expect(result.limitations).toBe(SOIL_MOISTURE_PERCENTILE_LIMITATIONS);
  });
  ...
  it("excludes the target's own year from the record without changing the rank", () => {
    ...
    const withOwnYear = [...record, soilMonth(2025, 7, 5)];
    const result = describeSoilMoisturePercentile(
      soilMonth(2025, 7, 130),
      withOwnYear,
      AVAILABLE_THROUGH
  ...
  it("passes through insufficient-samples without inventing a rank", () => {
    ...
    expect(result.isDriestInRecord).toBeNull();
  ...
  it("preserves the cited soil-moisture provenance through the baseline", () => {
    const record = priorYears(
      7,
      [100, 105, 110, 115, 120, 125, 135, 140, 145, 150]
    );
    ...
    expect(result.baseline.metric.source).toBe(
      CLIMATE_METRICS["soil-moisture"].source
    );
